...
class SMSClient(models.Model):
	...
	def _check_permissions(self):
		...
		print  "ggggggggggggggggggggggg",data
		if len(data) <= 0:
			return False
		return True
	...
	@api.multi
	def _prepare_smsclient_queue_2(self, data, name):
		return {
			'name': name,
	...
	@api.multi    
	def send_msg_2(self, data):
		if self._context is None:
			self._context = {}
		gateway_id = data['gateway']
		gateway = self.env['sms.smsclient'].browse(gateway_id)
		# raise UserError(str(gateway))
		if gateway:
			if not self._check_permissions():
			...
			queue_obj = self.env['sms.smsclient.queue']
			vals = self._prepare_smsclient_queue_2(data, name )
...
class SMSQueue(models.Model):
	...
	msg = fields.Text('SMS Text', size=256,
			required=True, readonly=True,
			states={'draft': [('readonly', False)]})
	mobile = fields.Char('Mobile No', size=256,readonly=True,
			states={'draft': [('readonly', False)]})
...
class partner_sms_send(models.Model):
	...
	def _default_get_mobile(self):
		if self._context is None:
			self._context = {}
		partner_pool = self.env['student.student']
